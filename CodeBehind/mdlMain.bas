...
Private Const AliquotProcessWrkSh = "Aliquot_Processing"
...
Function GetFirstFoundValue(val_to_find As String, col_letter As String, col_offset_num As String, Optional ws As Worksheet = Nothing) As String
    Dim val As String
    Dim rng As Range, SearchResult As Range
    Dim offSetcol As Integer, offSetrow As Integer

    If ws Is Nothing Then
        Set ws = Worksheets(SubAliquotBarcodesWrkSh)
    End If
    
    Set rng = ws.Range(col_letter & ":" & col_letter)
    
    offSetrow = 0
    offSetcol = col_offset_num
            
    On Error GoTo ErrorHandler
    
    Set SearchResult = rng.Find(What:=val_to_find, LookAt:=xlWhole, LookIn:=xlValues, MatchCase:=False)
...
Public Function PopulateAliquotProcessingTab(Optional copy_subaliquots_before_processing As Boolean = False)
    ...
    Dim rows_num As Integer, cols_num As Integer
    Dim target_sub_al_col As String
    Dim source_sub_al_col As String
    Dim source_al_col As String
    Dim sa_id As String, a_id As String
    Dim col_header As String, col_instr As String, cell_val As String
    Dim jVal As dictionary, json_str As String
    ...
    iResponse = MsgBox("The system is about to start processing aliquots on 'Aliquot_Processing' tab. " _
                ...
                & vbCrLf & vbCrLf & "Do you want to proceed? If not, click 'Cancel'." _
                , vbOKCancel + vbInformation, "Aliquot Processing Tool")
    ...
    With Worksheets(AliquotProcessWrkSh)
        'get a range (consisting of a single column) with all rows that needs to be processed
        Set rng_rows = .Range(target_sub_al_col & "2:" & target_sub_al_col & CStr(rows_num))
        
        'clear the range first
        'rng.ClearContents
        
        'loop through rows
        For Each row_cell In rng_rows
            sa_id = row_cell.Value2 'current sub-aliquot id value
            'current aliquot id value based on the provided sub-aliquot
            a_id = GetFirstFoundValue(sa_id, source_sub_al_col, _
                        CStr(GetConfigParameterValueB("SA_sheet - Aliquot ID - offset")), _
                        Worksheets(CStr(GetConfigParameterValueC("SA_sheet - Aliquot ID - offset"))))
            
            'get a range (consisting of a single row) correspondig to the row that is being currently processed
            Set rng_cols = .Range(Cells(row_cell.Row, row_cell.Column).Offset(0, 1).Address & ":" & Cells(row_cell.Row, cols_num).Address)
            'loop through columns
            For Each col_cell In rng_cols
...
'''                End Select
        ...
        .Activate 'bring focus to the "Aliquot_Processing" tab
        .Cells(1, 1).Activate 'bring focus to the first cell on the sheet
...
Public Function GetWorkSheetRowsCount(ws As Worksheet) As Integer
    ...
    GetWorkSheetRowsCount = ws.Cells.Find("*", SearchOrder:=xlByRows, SearchDirection:=xlPrevious).Row
End Function

Public Function GetWorkSheetColumnsCount(ws As Worksheet) As Integer
    'DetectionFileLoadedRows = Worksheets(DetectionFileWrkSh).Cells(Rows.Count, 1).End(xlUp).Row 'works only if last filled row in column is required; won't work if rows has different number of rows filled
    GetWorkSheetColumnsCount = ws.Cells.Find("*", SearchOrder:=xlByColumns, SearchDirection:=xlPrevious).Column
End Function
...
Function FindAllValuesOrLocationRows(rng As Range, _
    ...
    With rng
        ...
        Else
            ...
            'ReDim out_arr(0)
            out_arr = Split(Empty) ' creates an empty array
            'Debug.Print (ArrLength(out_arr))
        End If
    End With
    
    If ArrLength(out_arr) > 0 And sort_output Then
...
Public Sub ImportDetectionFile_todelete()
    Dim iResponse As Integer
...
Public Sub ImportSubAliquotBarcodesFile()
    Dim iResponse As Integer
    Dim importFileOutcome As Boolean
    Dim strFileToOpen As String
    Dim tStart As Date, tEnd As Date
    ...
    'confirm if user want to proceed.
    iResponse = MsgBox("The system is about to start importing a Sub-aliquot Barcodes file to the 'Sub-aliquot_barcodes' tab. " _
                & vbCrLf & "- Any currently existing data will be overwritten wih the new data!" _
                & vbCrLf & vbCrLf & "Do you want to proceed? If not, click 'Cancel'." _
                , vbOKCancel + vbInformation, "Sub-aliquot Barcode Processing")
    ...
    'select a file to be loaded
    strFileToOpen = Application.GetOpenFilename _
        (Title:="Please choose a Sub-aliquot Barcodes file to open", _
        FileFilter:="Excel Files *.xlsx* (*.xlsx*), Excel 2003 Files *.xls* (*.xls*),")
    ...
    importFileOutcome = ImportFileWhole(strFileToOpen, Worksheets(SubAliquotBarcodesWrkSh), GetConfigParameterValueB("Sub-aliquot Barcodes File Sheet Name"))
    
    If importFileOutcome Then
        'proceed here only if the file was successfully loaded
        Worksheets(SubAliquotBarcodesWrkSh).Activate 'bring focus to the "logs" tab
        Worksheets(SubAliquotBarcodesWrkSh).Cells(1, 1).Activate 'bring focus to the first cell on the sheet
...
Public Sub RefreshDBConnections(Optional hideConfirmation As Boolean = False)
    ...
    If Not hideConfirmation Then
        MsgBox "All database linked data was refreshed. Check results on the 'DB_Manifest_Data' tab.", vbInformation, "Sub-aliquot Processing Tool"
    End If
End Sub
...
Private Function GetDateReceived() As String
    ...
    cfg_value = GetConfigParameterValueB("Aliquot_Processing_Export_Date")
    
    If Len(Trim(cfg_value)) = 0 Then
...
'    End If
    ...
    'confirm if user want to proceed.
    iResponse = MsgBox("The system is about to start exporting validated data from 'Aliquot_Processing' tab." & vbCrLf _
    ...
    Set ws_source = Worksheets(AliquotProcessWrkSh) 'set reference to the "aliquot_processing" sheet
    ...
    If Not wb Is Nothing Then
        ...
        If ws_target.UsedRange.rows.Count > 1 Then
            ...
            str1 = "New Aliquot Processing export file was successfully created - " & wb.FullName '& ""
        ...
        Else
            ...
            str1 = "Newly created Aliquot Processing export file - " & wb.FullName & " - appears to be empty and will be deleted."
        ...
        MsgBox str1, msg_status, "Export of Processed Aliquots"
        
        If empty_file_flag Then
            'delete just created empty file
            If DeleteFile(new_file_name) Then
                str1 = "Newly created Aliquot Processing export file - " & new_file_name & " - was successfully deleted."
                'AddLogEntry str1, LogMsgType.warning, wb_source
            Else
                str1 = "The application was not able to delete the newly created Aliquot Processing export file - " & wb.FullName & "."
        ...
        Application.CutCopyMode = False 'clean clipboard
    Else
        str1 = "Existing file with the same name was present (" & new_file_name & "). " & vbCrLf & vbCrLf & "Creation of the new export file was skipped based on user's input."
        MsgBox str1, vbCritical, "Export of Detection Data"
        out_dict.Add "msg", str1
...
'    spec_prep = GetSpecimenPrepAbbr() 'get specimen abbreviation value
    ...
    GetFileNameToSave = "Aliquot_Processing_Tool_" & ship_date & Trim(post_fix) & ".xlsx"
End Function

Public Sub ShowVersionMsg()
    MsgBox "Aliquot Processing Tool - version #" & Version _
    & vbCrLf & vbCrLf _
